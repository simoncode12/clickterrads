<?php
// File: /admin/demand-report.php (FINAL & OPTIMIZED - Reads from summary table)
...
$partners_list = $conn->query("SELECT id, name FROM ssp_partners ORDER BY name ASC");

// --- Main Hybrid Query Construction ---
$group_by_field = "sp.name as group_field";
$group_by_clause = "GROUP BY T.ssp_partner_id, sp.name";
$main_column_header = "Demand Partner";

if ($group_by == 'date') {
    $group_by_field = "T.stat_date as group_field";
    $group_by_clause = "GROUP BY T.stat_date";
    $main_column_header = "Date";
} elseif ($group_by == 'country') {
...
$sql = "
    ...
    FROM (
        ...
        SELECT ssp_partner_id, stat_date, country, impressions AS total_impressions, clicks AS total_clicks, cost AS total_revenue
        FROM stats_daily_summary
        WHERE stat_date BETWEEN ? AND ? AND campaign_id = -1
        
        UNION ALL
        
        -- Data hari ini dari tabel mentah
        SELECT ssp_partner_id, stat_date, country, SUM(impressions) AS total_impressions, SUM(clicks) AS total_clicks, SUM(cost) AS total_revenue
        FROM campaign_stats
        WHERE stat_date = ? AND stat_date >= ? AND campaign_id = -1
        GROUP BY ssp_partner_id, stat_date, country
    ) AS T
    JOIN ssp_partners sp ON T.ssp_partner_id = sp.id
";
...
<div class="card mb-4">
    ...
    <div class="card-body">
        <form method="GET" class="row g-3 align-items-end">
            ...
            <div class="col-md-2"><a href="demand-report.php" class="btn btn-secondary w-100">Reset</a></div>
        </form>
    </div>
</div>

<div class="card">
    <div class="card-header"><i class="bi bi-graph-up-arrow me-2"></i>Performance Data</div>
    <div class="card-body">
        <div class="table-responsive"><table class="table table-striped table-bordered table-hover">
            <thead class="table-dark"><tr><th><?php echo $main_column_header; ?></th><th>Impressions Sold</th><th>Clicks</th><th>CTR</th><th>eCPM ($)</th><th>Revenue ($)</th></tr></thead>
            <tbody>
            ...
            <tfoot class="table-group-divider fw-bold">
                <tr><td>Total</td><td><?php echo number_format($totals['impressions']); ?></td><td><?php echo number_format($totals['clicks']); ?></td><td><?php echo number_format($total_ctr, 2); ?>%</td><td><?php echo number_format($total_ecpm, 4); ?></td><td><?php echo number_format($totals['revenue'], 6); ?></td></tr>
            </tfoot>
            <?php endif; ?>
        </table></div>
    </div>
